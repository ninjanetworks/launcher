...
/**
 * The workspace is a wide area with a wallpaper and a finite number of screens. Each screen contains a number of icons,
 * folders or widgets the user can interact with. A workspace is meant to be used with a fixed width only.
 */
public class Workspace extends WidgetSpace implements DropTarget, DragSource, DragScroller,
    ...
    private final int[][] distro = { { 1 }, { 2 }, { 1, 2 }, { 2, 2 }, { 2, 1, 2 }, { 2, 2, 2 }, { 2, 3, 2 },
                    { 3, 2, 3 }, { 3, 3, 3 } };
    ...
    public Workspace(Context context, AttributeSet attrs, int defStyle) {
        ...
        View screen = layoutInflter.inflate(R.layout.workspace_screen1, null, false);
        addView(screen);
        screen.findViewById(R.id.dialer).setOnClickListener(new OnClickListener() {
        ...
        screen = layoutInflter.inflate(R.layout.workspace_screen2, null, false);
        addView(screen);
        screen = layoutInflter.inflate(R.layout.workspace_screen3, null, false);
        addView(screen);
        screen = layoutInflter.inflate(R.layout.workspace_screen4, null, false);
        addView(screen);
        screen = layoutInflter.inflate(R.layout.workspace_screen5, null, false);
        addView(screen);
    ...
    private void initWorkspace() {
        ...
        mDesktopRows = 2;// AlmostNexusSettingsHelper.getDesktopRows(getContext());
        mDesktopColumns = 1;// AlmostNexusSettingsHelper.getDesktopColumns(getContext());
    ...
    /**
     * Adds the specified child in the current screen. The position and dimension of the child are defined by x, y,
     * spanX and spanY.
     * 
     * @param child
    ...
    /**
     * Adds the specified child in the specified screen. The position and dimension of the child are defined by x, y,
     * spanX and spanY.
     * 
     * @param child
     *            The child to add in one of the workspace's screens.
    ...
    void addInScreen(View child, int screen, int x, int y, int spanX, int spanY, boolean insert) {
        ...
        // /*
        // * Rogro82@xda Extended : Do not throw an exception else it will crash when there is an item on a hidden
        // * homescreen
        // */
        // return;
        ...
        // clearVacantCache();
        //
        // final ViewGroup group = (ViewGroup) getChildAt(screen);
        // ViewGroup.LayoutParams lp = child.getLayoutParams();
        // if (lp == null) {
        // lp = new ViewGroup.LayoutParams(x, y);
        // } else {
        // // lp.cellX = x;
        // // lp.cellY = y;
        // // lp.cellHSpan = spanX;
    ...
    /*
     * void addWidget(View view, Widget widget, boolean insert) { addInScreen(view, widget.screen, widget.cellX,
     * widget.cellY, widget.spanX, widget.spanY, insert); }
     */
    ...
    CellLayout.CellInfo findAllVacantCellsFromModel() {
        // ViewGroup group = (ViewGroup) getChildAt(mCurrentScreen);
        // if (group != null) {
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (mRevertInterpolatorOnScrollFinish) {
                ...
                /*
                 * if(mLauncher.isScrollableAllowed()){ View changedView; if (lastScreen != mCurrentScreen) {
                 * changedView = getChildAt(lastScreen); // A screen get out if (changedView instanceof
                 * WidgetCellLayout) ((WidgetCellLayout) changedView).onViewportOut(); } changedView =
                 * getChildAt(mCurrentScreen); // A screen get in if (changedView instanceof WidgetCellLayout)
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (!mSensemode) {
            ...
            if (fastDraw) {
                ...
                /*
                 * if (nextScreen >= 0 && nextScreen < getChildCount() && Math.abs(mCurrentScreen - nextScreen) == 1) {
                 * drawChild(canvas, currentView, drawingTime); drawChild(canvas, getChildAt(nextScreen), drawingTime);
                 * } else { // If we are scrolling, draw all of our children final int count = getChildCount(); for (int
                 * i = 0; i < count; i++) { drawChild(canvas, getChildAt(i), drawingTime); } }
                 */
            }
        } else {
            long currentTime;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (xMoved || yMoved) {
                // If xDiff > yDiff means the finger path pitch is smaller than 45deg so we assume the user want to
                // scroll X axis
                if (xDiff > yDiff) {
                ...
                // If yDiff > xDiff means the finger path pitch is bigger than 45deg so we assume the user want to
                // either scroll Y or Y-axis gesture
                else if (getOpenFolder() == null) {
                    // As x scrolling is left untouched (more or less untouched;)), every gesture should start by
                    // dragging in Y axis. In fact I only consider useful, swipe up and down.
        ...
        case MotionEvent.ACTION_UP:
            ...
            if ((mTouchState != TOUCH_STATE_SCROLLING) && (mTouchState != TOUCH_SWIPE_DOWN_GESTURE)
                            && (mTouchState != TOUCH_SWIPE_UP_GESTURE) && (mTouchState != TOUCH_DOUBLE_TAP_GESTURE)) {
                final ViewGroup currentScreen = (ViewGroup) getChildAt(mCurrentScreen);
                // if (!currentScreen.lastDownOnOccupiedCell()) {
                // getLocationOnScreen(mTempCell);
                // // Send a tap to the wallpaper if the last down was on empty space
                // if (lwpSupport) {
                // mWallpaperManager.sendWallpaperCommand(getWindowToken(), "android.wallpaper.tap", mTempCell[0]
                // + (int) ev.getX(), mTempCell[1] + (int) ev.getY(), 0, null);
                // }
                // }
            }
            // Release the drag
    ...
    void snapToScreen(int whichScreen) {
        ...
        // Log.d("Workspace",
        // "whichScreen: "+whichScreen+"; mCurrentScreen: "+mCurrentScreen+"; getChildCount: "+(getChildCount()-1));
        if (screenDelta == 0) {
            duration += 400;
        ...
        if (mScrollingLoop && (whichScreen == 0) && (mCurrentScreen == maxScreenIndex)) {
            delta = ((mCurrentScreen + 1) * getWidth()) - getScrollX(); // delta should be positive
            ...
            delta = ((mCurrentScreen - 1) * getWidth()) - getScrollX(); // delta should be negative (same as 0 - delta)
    ...
    private void onDropExternal(int x, int y, Object dragInfo, CellLayout cellLayout, boolean insertAtFirst) {
        ...
        case LauncherSettings.Favorites.ITEM_TYPE_LIVE_FOLDER:
            view = LiveFolderIcon.fromXml(R.layout.live_folder_icon, mLauncher, (ViewGroup) getChildAt(mCurrentScreen),
                            (LiveFolderInfo) info);
            break;
        default:
    ...
    /**
     * Return the current {@link CellLayout}, correctly picking the destination screen while a scroll is in progress.
     */
    ...
    public void onDropCompleted(View target, boolean success) {
        ...
        if (success) {
            if ((target != this) && (mDragInfo != null)) {
                final ViewGroup cellLayout = (ViewGroup) getChildAt(mDragInfo.screen);
     ...
     */
    /*
     * private Search findSearchWidget(CellLayout screen) { final int count = screen.getChildCount(); for (int i = 0; i
     * < count; i++) { View v = screen.getChildAt(i); if (v instanceof Search) { return (Search) v; } } return null; }
     */

    /**
     * Gets the first search widget on the current screen, if there is one. Returns <code>null</code> otherwise.
     */
    /*
     * public Search findSearchWidgetOnCurrentScreen() { CellLayout currentScreen =
     * (CellLayout)getChildAt(mCurrentScreen); return findSearchWidget(currentScreen); }
     */
    ...
    /**
     * Set true to allow long-press events to be triggered, usually checked by {@link Launcher} to accept or block
            ...
            for (int j = 0; j < childCount; j++) {
                ...
                if (tag instanceof ApplicationInfo) {
                    ...
                    if (tagInfo.id == info.id) {
                        ...
                        addInScreen(newview, info.screen, info.cellX, info.cellY, info.spanX, info.spanY, false);
                        break;
                    }
                } else if (tag instanceof UserFolderInfo) {
            ...
            for (int j = 0; j < childCount; j++) {
                ...
                if (tag instanceof ApplicationInfo) {
                    ...
                    if (((info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) || (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT))
                        ...
                        final Drawable icon = Launcher.getModel().getApplicationInfoIcon(mLauncher.getPackageManager(),
                                        info, mLauncher);
    ...
    /**
     * ADW: Make a local copy of wallpaper bitmap to use instead wallpapermanager only when detected not being a Live
     * Wallpaper
     */
    ...
    private RectF getScaledChild(View child) {
        ...
        for (int rows = 0; rows < distro[distro_set].length; rows++) {
            ...
            for (int columns = 0; columns < distro[distro_set][rows]; columns++) {
                ...
                if (child == c) {
                    return new RectF(leftMargin + xpos, topMargin + ypos, leftMargin + xpos + childWidth, topMargin
                                    + ypos + childHeight);
                } else {
                    xpos += childWidth;
                }
    ...
    public boolean setPositionAndScale(Object obj, PositionAndScale update, PointInfo touchPoint) {
        ...
        if ((targetZoom < 0) && (mStatus == SENSE_CLOSED)) { // Change to > 0 for pinch out, != 0 for both pinch in and
            // out.
            invalidate();
            return true;
        }
    ...
    public boolean isWidgetAtLocationScrollable(int x, int y) {
        ...
        // // (View)currentScreen.getChildAt(cell_xy[0]*currentScreen.getCountX etc etc); As the widget is positioned at
        // // the very first cell of the widgetspace
        // for (int i = 0; i < count; i++) {
        // View child = currentScreen.getChildAt(i);
        // if (child != null) {
    ...
    /**
     * ADW: hide live wallpaper to speedup the app drawer I think the live wallpaper needs to support the "hide" command
     * and not every LWP supports it.
     * http://developer.android.com/intl/de/reference/android/app/WallpaperManager.html#sendWallpaperCommand
     * %28android.os.IBinder,%20java.lang.String,%20int,%20int,%20int,%20android.os.Bundle%29
     * 
     * @param hide
     */
    public void hideWallpaper(boolean hide) {
        if ((getWindowToken() != null) && (mLauncher.getWindow() != null)) {
            if (hide) {
                mWallpaperManager.sendWallpaperCommand(getWindowToken(), "hide", 0, 0, 0, null);
            } else {
                mWallpaperManager.sendWallpaperCommand(getWindowToken(), "show", 0, 0, 0, null);
            }
        }
    }

    /**
     * ADW: Remove the specified screen and all the contents Almos update remaining screens content inside model
     * 
     * @param screen
     */
    ...
    // protected ViewGroup addScreen(int position) {
    // LayoutInflater layoutInflter = LayoutInflater.from(mLauncher);
    // ViewGroup screen = (ViewGroup) layoutInflter.inflate(R.layout.workspace_screen1, this, true);
    ...
    private void moveItemPositions(int screen, int diff) {
        ...
        // final ItemInfo item = (ItemInfo) tag;
        // if ((item != null) && (item.container == LauncherSettings.Favorites.CONTAINER_DESKTOP)) {
        // LauncherModel.moveItemInDatabase(mLauncher, item, item.container, item.screen + diff, item.cellX,
        // item.cellY);
        // }
        // }
        // }
    ...
    private void preTransitionDraw(Canvas canvas, int mTransitionStyle, boolean isMovingRight, int xOffset,
                    float transformAmount, int width) {
    ...
    private void postTransitionDraw(Canvas canvas, int mTransitionStyle, boolean isMovingRight, int xOffset,
                    float transformAmount, int width) {
        if (mTransitionStyle == 2) // rotate
        {
