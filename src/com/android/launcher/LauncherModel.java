...
public class LauncherModel {
    ...
    /**
     * Drop our cache of components to their lables & icons. We do this from Launcher when applications are
     * added/removed. It's a bit overkill, but it's a rare operation anyway.
     */
     ...
     * 
     * @return true if the applications loader must be started (see startApplicationsLoader()), false otherwise.
     */
    synchronized boolean loadApplications(boolean isLaunching, Launcher launcher, boolean localeChanged) {
        ...
        if (isLaunching && mApplicationsLoaded && !localeChanged) {
            mApplicationsAdapter = new ApplicationsAdapter(launcher, mApplications, AppCatalogueFilters.getInstance()
                            .getDrawerFilter());
            if (DEBUG_LOADERS) {
    ...
    private boolean addEnabledAndUpdateActivities(List<ResolveInfo> matches, ApplicationsAdapter adapter,
                    Launcher launcher) {
        ...
        for (int i = 0; i < count; i++) {
            final ResolveInfo info = matches.get(i);
            final ApplicationInfo applicationInfo = findIntent(adapter, info.activityInfo.applicationInfo.packageName,
                            info.activityInfo.name);
            if (applicationInfo == null) {
    ...
    private static ApplicationInfo findIntent(ApplicationsAdapter adapter, String packageName, String name) {
    ...
    private static ApplicationInfo makeAndCacheApplicationInfo(PackageManager manager,
                    HashMap<ComponentName, ApplicationInfo> appInfoCache, ResolveInfo info, Context context) {
        ...
        if (application == null) {
            ...
            application.setActivity(componentName, Intent.FLAG_ACTIVITY_NEW_TASK
    ...
    private class ApplicationsLoader implements Runnable {
        ...
        public void run() {
            ...
            /*
             * If we've made it this far and mStopped isn't set, we've successfully loaded applications. Otherwise,
             * applications aren't loaded.
             */
    ...
    /**
     * Loads all of the items on the desktop, in folders, or in the dock. These can be apps, shortcuts or widgets
     */
    void loadUserItems(boolean isLaunching, Launcher launcher, boolean localeChanged, boolean loadApplications) {
        if (DEBUG_LOADERS) {
            d(LOG_TAG, "loading user items in " + Thread.currentThread().toString());
        }
        ...
        if (isLaunching && isDesktopLoaded()) {
            ...
            if (DEBUG_LOADERS) {
                d(LOG_TAG, "  --> loading from cache: " + mDesktopItems.size() + ", " + mDesktopAppWidgets.size());
            }
        ...
        mDesktopItemsLoaded = false;
        mDesktopItemsLoader = new DesktopItemsLoader(launcher, localeChanged, loadApplications, isLaunching);
                ...
                if (id == Id) {
                    ...
                    if ((itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION)
                        ...
                        if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                            ...
                            info = getApplicationInfoShortcut(c, context, iconTypeIndex, iconPackageIndex,
    ...
    private class DesktopItemsLoader implements Runnable {
        ...
        DesktopItemsLoader(Launcher launcher, boolean localeChanged, boolean loadApplications, boolean isLaunching) {
                    ...
                    try {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                            ...
                            if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                                ...
                                info = getApplicationInfoShortcut(c, launcher, iconTypeIndex, iconPackageIndex,
                                                iconResourceIndex, iconIndex);
                            }
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_LIVE_FOLDER:
                            ...
                            loadLiveFolderIcon(launcher, c, iconTypeIndex, iconPackageIndex, iconResourceIndex,
                                            liveFolderInfo);
                            ...
                            break;
                        /*
                         * case LauncherSettings.Favorites.ITEM_TYPE_WIDGET_SEARCH: widgetInfo = Widget.makeSearch();
                         * 
                         * container = c.getInt(containerIndex); if (container !=
                         * LauncherSettings.Favorites.CONTAINER_DESKTOP) { e(Launcher.LOG_TAG,
                         * "Widget found where container " + "!= CONTAINER_DESKTOP  ignoring!"); continue; }
                         * 
                         * widgetInfo.id = c.getLong(idIndex); widgetInfo.screen = c.getInt(screenIndex);
                         * widgetInfo.container = container; widgetInfo.cellX = c.getInt(cellXIndex); widgetInfo.cellY =
                         * c.getInt(cellYIndex); widgetInfo.spanX = c.getInt(spanXIndex); widgetInfo.spanY =
                         * c.getInt(spanYIndex);
                         * 
                         * desktopItems.add(widgetInfo); break;
                         */
        ...
        private void updateShortcutLabels(ContentResolver resolver, PackageManager manager) {
            final Cursor c = resolver
                            .query(LauncherSettings.Favorites.CONTENT_URI, new String[] {
                                            LauncherSettings.Favorites._ID, LauncherSettings.Favorites.TITLE,
                            ...
                            if (Intent.ACTION_MAIN.equals(shortcut.getAction())) {
                                ...
                                if (name != null) {
                                    ...
                                    if ((title == null) || !title.equals(label)) {
                                        ...
                                        resolver.update(LauncherSettings.Favorites.CONTENT_URI_NO_NOTIFICATION, values,
                                                        "_id=?", new String[] { String.valueOf(c.getLong(idIndex)) });
    ...
    static private void loadFolderIcon(Context context, Cursor c, int iconTypeIndex, int iconIndex,
        ...
        if (iconType == LauncherSettings.Favorites.ICON_TYPE_BITMAP) {
            ...
            try {
                ...
                folderInfo.icon = new FastBitmapDrawable(Utilities.createBitmapThumbnail(bitmap, context));
    ...
    private static void loadLiveFolderIcon(Launcher launcher, Cursor c, int iconTypeIndex, int iconPackageIndex,
                    int iconResourceIndex, LiveFolderInfo liveFolderInfo) {
    ...
    /**
     * Return an existing UserFolderInfo object if we have encountered this ID previously, or make a new one.
     */
    ...
    /**
     * Fills in the occupied structure with all of the shortcuts, apps, folders and widgets in the model.
     */
     ...
     */
    private static ApplicationInfo getApplicationInfo(PackageManager manager, Intent intent, Context context) {
     ...
     */
    private static ApplicationInfo getApplicationInfoShortcut(Cursor c, Context context, int iconTypeIndex,
    ...
    static boolean shortcutExists(Context context, String title, Intent intent) {
        final ContentResolver cr = context.getContentResolver();
        Cursor c = cr.query(LauncherSettings.Favorites.CONTENT_URI, new String[] { "title", "intent" },
                        "title=? and intent=?", new String[] { title, intent.toUri(0) }, null);
    ...
    /**
     * Add an item to the database in a specified container. Sets the container, screen, cellX and cellY fields of the
     * item. Also assigns an ID to the item.
     */
        ...
        if (themePackage.equals(Launcher.THEME_DEFAULT)) {
            ...
            if (AlmostNexusSettingsHelper.getThemeIcons(context)) {
                ...
                if (themeResources != null) {
                    ...
                    // use IconShader
                    if (icon == null) {
                        if ((compiledIconShaderName == null) || (compiledIconShaderName.compareTo(themePackage) != 0)) {
        ...
        if ((packageName != null) && (packageName.length() > 0) && (mApplicationsAdapter != null)) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if ((info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION)
                                && packageName.equals(name.getPackageName()) && (info.counter != counter)) {
