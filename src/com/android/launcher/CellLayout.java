...
public class CellLayout extends WidgetCellLayout {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        /*
         * if (mOccupied == null) { if (mPortrait) { mOccupied = new boolean[mShortAxisCells][mLongAxisCells]; } else {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (!found) {
                ...
                cellInfo.spanX = 1;
                cellInfo.spanY = 1;
                cellInfo.valid = (cellXY[0] >= 0) && (cellXY[1] >= 0) && (cellXY[0] < xCount) && (cellXY[1] < yCount)
    ...
    private static void findIntersectingVacantCells(CellInfo cellInfo, int x, int y, int xCount, int yCount,
                    boolean[][] occupied) {
     ...
     * {
     * 
     * addVacantCell(current, cellInfo); if (current.left > 0) { if (isColumnEmpty(current.left - 1, current.top,
     * current.bottom, occupied)) { current.left--; findVacantCell(current, xCount, yCount, occupied, cellInfo);
     * current.left++; } }
     * 
     * if (current.right < xCount - 1) { if (isColumnEmpty(current.right + 1, current.top, current.bottom, occupied)) {
     * current.right++; findVacantCell(current, xCount, yCount, occupied, cellInfo); current.right--; } }
     * 
     * if (current.top > 0) { if (isRowEmpty(current.top - 1, current.left, current.right, occupied)) { current.top--;
     * findVacantCell(current, xCount, yCount, occupied, cellInfo); current.top++; } }
     * 
     * if (current.bottom < yCount - 1) { if (isRowEmpty(current.bottom + 1, current.left, current.right, occupied)) {
     * current.bottom++; findVacantCell(current, xCount, yCount, occupied, cellInfo); current.bottom--; } } }
     */
    // TODO: ADW.
    /**
     * I don't understand at all why the "findVacantCell" recursive method do what it does, but seems there's something
     * wrong with it actually works cause default launchers use a 4x4 grid, but as soon as we rise it to 6x6 or 7x7
     * (please, think on tablets!!!!) it starts to ANR and act weird
     * 
     * Tried a lot of things, and @unekual sent me the following non-recursive piece of code This does not find ALL the
     * possible vacant cell combinations, but it works right fine and does not stuck on a stupid ANR.
     */
    ...
    /**
     * Variant of findAllVacantCells that uses LauncerModel as its source rather than the views.
     */
    CellInfo findAllVacantCellsFromOccupied(boolean[][] occupied, final int xCount, final int yCount) {
        ...
        /*
         * for (int x = 0; x < xCount; x++) { for (int y = 0; y < yCount; y++) { if (!occupied[x][y]) { current.set(x,
         * y, x, y); findVacantCell(current, xCount, yCount, occupied, cellInfo); occupied[x][y] = true; } } }
         */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int topExtra = 0;
        if (AlmostNexusSettingsHelper.getDesktopIndicator(getContext())
                        && (AlmostNexusSettingsHelper.getDesktopIndicatorType(getContext()) == DesktopIndicator.INDICATOR_TYPE_PAGER)) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mPortrait) {
                lp.setup(cellWidth, cellHeight, mWidthGap, mHeightGap, shortAxisStartPadding, longAxisStartPadding,
                                AlmostNexusSettingsHelper.getAutosizeIcons(getContext()));
            } else {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (lp.dropped) {
                    ...
                    mWallpaperManager.sendWallpaperCommand(getWindowToken(), "android.home.drop", cellXY[0] + childLeft
                                    + lp.width / 2, cellXY[1] + childTop + lp.height / 2, 0, null);
                }
            }
        }
    }
    ...
    /**
     * Find a vacant area that will fit the given bounds nearest the requested cell location. Uses Euclidean distance to
     * score multiple vacant areas.
     * 
     * @param pixelX
     ...
     * @return The X, Y cell of a vacant area that can contain this object, nearest the requested location.
     */
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public void setup(int cellWidth, int cellHeight, int widthGap, int heightGap, int hStartPadding,
                        int vStartPadding, boolean autoStretch) {
    ...
    static final class CellInfo implements ContextMenu.ContextMenuInfo {
        /**
         * See View.AttachInfo.InvalidateInfo for futher explanations about the recycling mechanism. In this case, we
         * recycle the vacant cells instances because up to several hundreds can be instanciated when the user long
         * presses an empty cell.
         */
        ...
        /**
         * This method can be called only once! Calling #findVacantCellsFromOccupied will restore the ability to call
         * this method.
         * 
         * Finds the upper-left coordinate of the first rectangle in the grid that can hold a cell of the specified
         * dimensions.
         * 
         * @param cellXY
         *            The array that will contain the position of a vacant cell if such a cell can be found.
         * @param spanX
