...
public class LauncherProvider extends ContentProvider {
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        /**
         * Send notification that we've deleted the {@link AppWidgetHost}, probably as part of the initial database
         * creation. The receiver may want to re-call {@link AppWidgetHost#startListening()} to ensure callbacks are
         * correctly set.
         */
        ...
        public void onCreate(SQLiteDatabase db) {
            if (LOGD) {
                Log.d(LOG_TAG, "creating new launcher database");
            }

            db.execSQL("CREATE TABLE favorites (" + "_id INTEGER PRIMARY KEY," + "title TEXT," + "intent TEXT,"
                            + "container INTEGER," + "screen INTEGER," + "cellX INTEGER," + "cellY INTEGER,"
                            + "spanX INTEGER," + "spanY INTEGER," + "itemType INTEGER,"
                            + "appWidgetId INTEGER NOT NULL DEFAULT -1," + "isShortcut INTEGER," + "iconType INTEGER,"
                            + "iconPackage TEXT," + "iconResource TEXT," + "icon BLOB," + "uri TEXT,"
                            + "displayMode INTEGER" + ");");

            db.execSQL("CREATE TABLE gestures (" + "_id INTEGER PRIMARY KEY," + "title TEXT," + "intent TEXT,"
                            + "itemType INTEGER," + "iconType INTEGER," + "iconPackage TEXT," + "iconResource TEXT,"
                            + "icon BLOB" + ");");
        ...
        private boolean convertDatabase(SQLiteDatabase db) {
            if (LOGD) {
                Log.d(LOG_TAG, "converting database from an older format, but not onUpgrade");
            }
            ...
            if (converted) {
                ...
                if (LOGD) {
                    Log.d(LOG_TAG, "converted and now triggering widget upgrade");
                }
                convertWidgets(db);
            }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            if (LOGD) {
                Log.d(LOG_TAG, "onUpgrade triggered");
            }
            ...
            if (version < 4) {
                ...
                try {
                    db.execSQL("CREATE TABLE gestures (" + "_id INTEGER PRIMARY KEY," + "title TEXT," + "intent TEXT,"
                                    + "itemType INTEGER," + "iconType INTEGER," + "iconPackage TEXT,"
                                    + "iconResource TEXT," + "icon BLOB" + ");");
                    db.setTransactionSuccessful();
                    version = 4;
                } catch (SQLException ex) {
                    // Old version remains, which means we wipe old data
            ...
            if (version < 5) {
                ...
                try {
                    ...
                    db.execSQL("INSERT INTO 'favorites'" + "(" + LauncherSettings.Favorites.TITLE + ","
                                    + LauncherSettings.Favorites.INTENT + "," + LauncherSettings.Favorites.CONTAINER
                                    + "," + LauncherSettings.Favorites.SCREEN + "," + LauncherSettings.Favorites.CELLX
                                    + "," + LauncherSettings.Favorites.CELLY + "," + LauncherSettings.Favorites.SPANX
                                    + "," + LauncherSettings.Favorites.SPANY + ","
                                    + LauncherSettings.Favorites.ITEM_TYPE + ","
                                    + LauncherSettings.Favorites.APPWIDGET_ID + ","
                                    + LauncherSettings.Favorites.ICON_TYPE + ","
                                    + LauncherSettings.Favorites.ICON_PACKAGE + ","
                                    + LauncherSettings.Favorites.ICON_RESOURCE + "," + LauncherSettings.Favorites.ICON
                                    + "," + LauncherSettings.Favorites.URI + ","
                                    + LauncherSettings.Favorites.DISPLAY_MODE + ")" + "VALUES(" + "'Apps',"
                                    + "'#Intent;action=" + CustomShirtcutActivity.ACTION_LAUNCHERACTION + ";component="
                                    + c.flattenToString() + ";i.DefaultLauncherAction.EXTRA_BINDINGVALUE=4;end',"
                                    + LauncherSettings.Favorites.CONTAINER_MAB + "," + "-1," + "-1," + "-1," + "1,"
                                    + "1," + "1," + "-1," + "0," + "'" + c.getPackageName() + "'," + "'"
                                    + c.getPackageName() + ":drawable/all_apps_button'," + "NULL," + "NULL," + "NULL);");
                    db.setTransactionSuccessful();
                    version = 5;
                } catch (SQLException ex) {
                    // Old version remains, which means we wipe old data
        ...
        /**
         * Upgrade existing clock and photo frame widgets into their new widget equivalents. This method allocates
         * appWidgetIds, and then hands off to LauncherAppWidgetBinder to finish the actual binding.
         */
        private void convertWidgets(SQLiteDatabase db) {
            ...
            try {
                ...
                if (LOGD) {
                    Log.d(LOG_TAG, "found upgrade cursor count=" + c.getCount());
                }
                ...
                while ((c != null) && c.moveToNext()) {
                    ...
                    try {
                        ...
                        if (LOGD) {
                            Log.d(LOG_TAG, "allocated appWidgetId=" + appWidgetId + " for favoriteId=" + favoriteId);
                        }
        ...
        /**
         * Launch the widget binder that walks through the Launcher database, binding any matching widgets to the
         * corresponding targets. We can't bind ourselves because our parent process can't obtain the BIND_APPWIDGET
         * permission.
         */
        ...
        /**
         * Gets an appwidget provider from the given package. If the package contains more than one appwidget provider,
